...
After this, you are good to go. In order to build a neural network load the keras-package with ``` library(keras)```. However, do `r htmltools::HTML("{{< hl-text danger >}} NOT {{< /hl-text >}}")` run ```install_keras(tensorflow = "gpu")``` because titis already installed and running this command will probably screw stuff up (like it did for me).  
